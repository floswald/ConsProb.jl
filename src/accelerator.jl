...
function resetAccel!(a::Accelerator)
	a.i = 1
end
...
function linearapprox2(x::Array{Float64,1},y::Array{Float64,1},xi::Float64,n::Int,acc::Accelerator)
	r = 0.0
	...
	# 	setAccel!(acc,searchsortedlast(x,xi,1,acc.i,Base.Forward))
	elseif xi >= x[acc.i+1]
		setAccel!(acc,searchsortedlast(x,xi,acc.i+1,n,Base.Forward))
	# else
		# hit!
	end
	@inbounds r = (y[acc.i] * (x[acc.i+1] - xi) + y[acc.i+1] * (xi - x[acc.i]) ) / (x[acc.i+1] - x[acc.i])
